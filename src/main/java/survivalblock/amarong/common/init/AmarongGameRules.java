...
public sealed interface AmarongGameRules permits AmarongGameRules.Dummy {

    GameRules.Key<GameRules.BooleanRule> FLYING_TICKETS_DROP =
            GameRuleRegistry.register("amarong:flyingTicketsDrop", GameRules.Category.MISC, GameRuleFactory.createBooleanRule(true));
    GameRules.Key<DoubleRule> BOOMERANG_DAMAGE =
    ...
    GameRules.Key<GameRules.BooleanRule> VERYLONGSWORD_PASSIVE_CHARGE =
    ...
    GameRules.Key<GameRules.BooleanRule> OBSCURE_SPAWNS_PARTICLES =
    ...
    record Dummy() implements AmarongGameRules {
    }
}
