...
public class AmarongHammerTwirlCompat {
    ...
    public static boolean hasTwirl(ItemStack stack) {
        return AmarongUtil.getLevel(stack, Twirl.TWIRLING) > 0;
    }
    ...
    public static void whack(PlayerEntity player) {
        ...
        int level = AmarongUtil.getLevel(stack, Twirl.TWIRLING);
        if (level <= 0) {
            return;
        }
        List<Entity> entities = new ArrayList<>();
        AmarongUtil.getEntitiesWithScaling(serverWorld, entity -> AmarongUtil.getProperlyScaledBox(player).expand(2).intersects(AmarongUtil.getProperlyScaledBox(entity)), entities);
        //noinspection RedundantCollectionOperation
        if (entities.contains(player)) {
            entities.remove(player);
        }
        DamageSource source = serverWorld.getDamageSources().playerAttack(player);
        entities.forEach(entity -> entity.damage(source, (entity instanceof PlayerEntity ? 0.5f : 0.25f) * (float) level));
    }
}
